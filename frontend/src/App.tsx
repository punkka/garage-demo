...
import { fetchAdminParked, fetchSpaces, fetchStatus, enterCar, exitCar } from './api';
import { AdminParkingResponse, Space } from './types';

const formatCost = (cents: number) => `€${(cents / 100).toFixed(2)}`;

function App() {
  const [spaces, setSpaces] = useState<Space[]>([]);
  const [status, setStatus] = useState({ availableSpaces: 0, isFull: false });
  ...
  const refreshDashboard = async () => {
    setLoading(true);
    try {
      const [statusResponse, spaceList, adminResponse] = await Promise.all([
        fetchStatus(),
        fetchSpaces(),
        fetchAdminParked(),
      ]);
  ...
  const handleEntry = async () => {
    ...
    try {
      ...
      setEntryError(error instanceof Error ? error.message : 'Unable to register entry.');
    }
  };
    ...
    <Container maxWidth="lg" className="app-shell">
      ...
      <Box mt={4}>
        ...
        <Typography variant="body1" gutterBottom>
          Available spaces: {adminData?.availableSpaces ?? '—'}
        </Typography>
        {adminData?.activeCars.length ? (
          adminData.activeCars.map((car) => (
            <Card key={`${car.spaceNumber}-${car.licensePlate}`} className="admin-card">
              <CardContent>
                <Typography variant="subtitle1">Space {car.spaceNumber}</Typography>
